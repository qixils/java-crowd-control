...
public class CrowdControl {
	private final Map<String, Function<Request, Response.ResultWrapper>> effectHandlers = new HashMap<>();
	...
	public static final Gson GSON = new GsonBuilder()
			...
			.registerTypeAdapter(Response.Result.class, new EnumOrdinalAdapter<>())
			.create();
	 ...
	 *     <li>the return type {@link Response.Result}</li>
	 * </ul>
	...
	public void registerHandlers(@NotNull Object object) {
		...
		for (Method method : clazz.getMethods()) {
			if (Modifier.isPublic(method.getModifiers()) // ensure method is public
					&& (method.getReturnType() == Response.Result.class || method.getReturnType() == Response.ResultWrapper.class) // validate return type
				...
				registerHandler(effect, request -> {
					Response.ResultWrapper output;
					try {
						Object result = method.invoke(object, request);
						if (result instanceof Response.Result res) {
							output = new Response.ResultWrapper(res);
						} else if (result instanceof Response.ResultWrapper wrap) {
							output = wrap;
						} else {
							output = new Response.ResultWrapper(Response.Result.UNAVAILABLE, "Unknown result type");
						}
					} catch (IllegalAccessException | InvocationTargetException e) {
						logger.log( Level.WARNING,"Failed to invoke method handler for effect \"" + effect + "\"", e);
						output = new Response.ResultWrapper(Response.Result.FAILURE, "Failed to invoke method handler");
					}
					return output;
				});
			}
	 ...
	 */
	public void registerHandler(@NotNull String effect, @NotNull Function<Request, Response.ResultWrapper> handler) {
	 ...
	 * A resulting value of {@code false} will result in an {@link Response.Result#UNAVAILABLE UNAVAILABLE} response packet.
	 * <p>
	 ...
	 */
	@NotNull
	public Response.ResultWrapper handle(@NotNull Request request) {
		for (Supplier<Boolean> check : globalChecks) {
			if (!check.get()) {
				return new Response.ResultWrapper(Response.Result.UNAVAILABLE, "The game is unavailable");
			}
		}

		String effect = Objects.requireNonNull(request, "request").getEffect();
		if (!effectHandlers.containsKey(effect)) {
			return new Response.ResultWrapper(Response.Result.UNAVAILABLE, "The effect couldn't be found");
		}
		...
		try {
			...
			return new Response.ResultWrapper(Response.Result.FAILURE, "The effect encountered an exception");
		}
	}
